...
declare var $: any;
@Component({
  selector: 'app-catalog-expiration-alert',
  templateUrl: './catalog-expiration-alert.component.html',
  styleUrls: ['./catalog-expiration-alert.component.css']
})
export class CatalogExpirationAlertComponent implements OnInit, OnDestroy {
  private subscription: Subscription;
  public catae$: Observable<CatalogExpirationAlert[]>;
  public acatae: CatalogExpirationAlert[];
      ...
      (params: Params) => {
        if (params['id']) {
          ...
          this.plannerService.getMeNumber().subscribe(data => {
            this.meRule = data;
            if (data !== undefined)
              if(data.Rule == "Catalog Expiration")
                this.setMeRule(data);
          })
        }
      })
  ...
  setMeRule(valObj: MeNumberRule) {
    ...
    this.catae$ = this.plannerService.getPQCatalogExpirationAlertByME(this.meRule.ME_PART_NUMBER);
    
    this.catae$.subscribe(x => {
      this.acatae = x;
      x.forEach(y => {
      ...
      if (this.acatae.length == yesCount)
    ...
    onSubmit(form: NgForm) {
      ...
      this.acatae.forEach(x => { 
        if (form.controls[x.CHKKEY].value == true) { 
          stations.push(x.CHKKEY);
        }
      });

      let catalogExpirationAlertReq = new CommentReq();
      catalogExpirationAlertReq.PLANNER_NAME = this.user.display_name;
      catalogExpirationAlertReq.Comment = form.controls.txtComment.value;
      catalogExpirationAlertReq.MENumber = this.meRule.ME_PART_NUMBER;
      catalogExpirationAlertReq.REVIEW_REASON = "Catalog Expiration";
      catalogExpirationAlertReq.STATIONS = stations;

      if (form.controls.txtComment.value.length)
        this.plannerService.insertPQAlert(catalogExpirationAlertReq)
          .subscribe(result => { 
            return this.resp = result;
          });
    
        // if (form.controls.chkDrop.value) {
        //   this.plannerService.dropOffPQCatalogExpirationReqAlert(catalogExpirationAlertReq.MENumber)
        //     .subscribe(result => {
        //     return this.resp = result;
        //     });
        ...
        //     updatepqmasterreq.REVIEW_REASON = catalogExpirationAlertReq.REVIEW_REASON
...
dropMENumber(menumber: string) {
  if (menumber) { 
    ...
    updatepqmasterreq.REVIEW_REASON = "Catalog Expiration";
    this.plannerService.updatePQMaster(updatepqmasterreq).subscribe(result => {
      this.showHideComments();
