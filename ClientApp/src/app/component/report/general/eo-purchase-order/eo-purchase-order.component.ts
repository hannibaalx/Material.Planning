...
@Component({
  selector: 'app-eo-purchase-order',
  templateUrl: './eo-purchase-order.component.html',
  styleUrls: ['./eo-purchase-order.component.css'],
  providers: [
...
export class EoPurchaseOrderComponent implements OnInit, OnDestroy {
  ...
  postartdate = new FormControl(moment());
  ...
  ngOnInit() {
    this.postations$ = this.reportService.getPoStations();
    this.postatus$ = this.reportService.getPoStatus();
  }
    ...
    switch (type) { 
      case "men":
        if (value.target.value.length > 9) {
          // change this
          this.filteredPoMEN$ = this.reportService.autoSearchPurchaseOrder(value.target.value, type);
          this.hidePoMEN= true;
        }
        else { 
      ...
      case "mpn":
        if (value.target.value.length > 4) {
          // change this
          this.filteredPoMPN$ = this.reportService.autoSearchPurchaseOrder(value.target.value, type);
          this.hidePoMPN = true;
        }
        else { 
      ...
      case "pon":
        if (value.target.value.length > 4) {
          // change this
          this.filteredPON$ = this.reportService.autoSearchPurchaseOrder(value.target.value, type);
          this.hidePON = true;
        }
        else { 
          this.hidePON = false;
  ...
  generateReport() {
    ...
    this.reportService.downloadReportPurchaseOrder(this.eoReportPurchaseOrderReq)
      .subscribe(result => {
